...
on this motivation, we introduce the \textit{Word
  Definitions Language Model Probing} (WDLMPro) dataset\footnote{WDLAMPro and evaluation scripts are available at \href{https://www.cis.lmu.de/~lksenel/WDLAMPro.zip}{https://www.cis.lmu.de/\texttildelow lksenel/WDLAMPro.zip}}; it
...
these words to have similar distributional characteristics
...
It is infeasible to evaluate human performance on the entirety of WDLAMPro. However, we provide a comparison with humans on a small subset of WDLAMPro to provide an intuition about the difficulty of the task. For each of the tasks, 20 synset groups that have maximum 10 candidates are randomly sampled from WDLAMPro. Then 2 native English speakers are asked to rank the candidates. Table \ref{tab:human_eval} displays the average performance of the human participants as well as the language models on this subset. For both of the tasks, performance of the best model is comparable to the average human performance.
...
\textbf{Acknowledgements.} We thank
...
%      nod.v.01 & express or signifasy by nodding \\\hline
